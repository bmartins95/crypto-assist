...
- [ ] T003 [US1] Add `backend/tests/test_config.py` with unit tests covering: valid origin passes, empty string raises, missing scheme raises, trailing slash raises, whitespace-wrapped origin is stripped and passes

**Checkpoint**: `Settings(frontend_origin='https://example.com')` passes; `Settings(frontend_origin='example.com')` raises `ValidationError`. Existing `pytest` suite still green.
...
- [ ] T004 [US2] Add `# nosec B608` comment to `backend/app/routes/ops.py` line 47 (`f"INSERT INTO ops ..."`) and line 67 (`f"UPDATE ops SET ..."`) per research.md Decision 2
- [ ] T005 [P] [US2] Run `cd backend && bandit -r app/ -ll` and confirm exit 0; if new findings appear, either fix the code or add a justified nosec comment and list the suppression in the PR description
- [ ] T006 [P] [US2] Run `cd backend && pip-audit` and confirm exit 0; if any high/medium CVEs are reported with no fix available, list each CVE ID and justification in the PR description per FR-010
...
- [ ] T007 [US3] Add `.next/**` to the `ignores` array in `web/eslint.config.mjs` per research.md Decision 3; the `.next/` directory is a stale Next.js build artefact causing hundreds of false-positive findings
- [ ] T008 [P] [US3] Run `cd web && npm run lint` and confirm exit 0 with no errors (warnings on third-party code are acceptable)
- [ ] T009 [P] [US3] Run `cd web && npm audit --audit-level=high` and confirm exit 0; if any high CVEs are reported with no fix available, list each CVE ID and justification in the PR description per FR-010
...
- [ ] T010 [US4] Verify `backend/AGENTS.md` "Token storage accepted risk" section covers the XSS threat model, current mitigations enforced by ESLint in CI, and guidance for future contributors; update the section if any of these three elements is missing or incomplete
...
- [ ] T011 [P] Run `cd backend && pytest` and confirm all tests pass with ≥90% coverage on `app/config.py`; paste the coverage summary output in the PR description
- [ ] T012 [P] Run `cd web && npm test` and confirm all tests pass; paste the coverage summary in the PR description
- [ ] T013 Verify the CI pipeline definition in `.github/workflows/deploy.yml` contains all four security steps: `bandit -r app/ -ll`, `pip-audit`, `npm run lint`, `npm audit --audit-level=high` in the correct order (after `pytest` / `npm test`)
...
- US1: T002 (validator code) must complete before T003 (tests are written to test T002)
- US2: T004 (add nosec) must complete before T005 (bandit run will fail without it)
- US3: T007 (ESLint ignore) must complete before T008 (lint run will fail without it)
- US4: T010 is standalone

### Parallel Opportunities

T005 and T006 can run in parallel (different tools, same backend).
T008 and T009 can run in parallel (different tools, same web package).
...
Parallel: T005 "Run bandit -r app/ -ll" AND T006 "Run pip-audit"

# After T007 completes:
Parallel: T008 "Run npm run lint" AND T009 "Run npm audit --audit-level=high"

# Final parallel:
Parallel: T011 "Run pytest" AND T012 "Run npm test"
```
...
2. Complete T002–T003 (FR-011 validator + tests)
3. **STOP and VALIDATE**: `pytest` passes; `Settings` fails fast on bad origin
4. Continue to US2, US3, US4 in order

### Full Delivery Order

1. T001 → T002 → T003 (US1: CORS complete)
2. T004 → T005 + T006 (US2: bandit + pip-audit clean)
3. T007 → T008 + T009 (US3: ESLint + npm audit clean)
4. T010 (US4: docs verified)
5. T011 + T012 → T013 (Polish: all tests pass, CI verified)
...
- The `# nosec B608` suppressions (T004) must be individually listed in the PR description per SC-004
