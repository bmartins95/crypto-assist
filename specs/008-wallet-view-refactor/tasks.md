...
- [ ] T001 Add `wallet_subtitle` to `UIText` in `shared/src/i18n/types.ts` and to all 10 locale files in `shared/src/i18n/locales/*.ts` (accurate translation of "Real-time quotes via CoinGecko · BRL"-style subtitle, adapted per locale's currency framing already used elsewhere in that locale file)
...
- [ ] T003 [P] Create `web/src/components/MetricCard.tsx` per contracts/components.md: props `{ label: string; value: string; valueColor?: 'pos' | 'neg'; sub?: string; subColor?: 'pos' | 'neg' }`; renders `.mcard > .ml + .mv[.pos|.neg] [+ .msub[.pos|.neg]]`; no internal state or formatting logic
- [ ] T004 [P] Create `web/src/components/ContentHeader.tsx` per contracts/components.md: props `{ title: string; subtitle: string; children?: React.ReactNode }`; renders `.chead > (.ct title + .cs subtitle) + .refresh (children)`
- [ ] T005 [P] Create `web/src/components/MetricCard.test.tsx`: renders label/value, applies `.pos`/`.neg` to value and sub when provided, omits color classes when not provided, renders without `sub`
- [ ] T006 [P] Create `web/src/components/ContentHeader.test.tsx`: renders title/subtitle, renders children in the actions area, renders with no children
...
- [ ] T010 [US2] Update `web/src/components/WalletTab.test.tsx`: add assertions that the header renders `t.nav_wallet`/`t.wallet_subtitle`, that the `statusMsg` prop text is visible, and that clicking the refresh button calls `onFetchPrices`
...
- [ ] T013 [P] Locale sweep: verify no missing-key fallback for `wallet_subtitle` across all 10 locales (compiler-enforced; run `npx tsc --noEmit` in `web/` and `shared/`)
- [ ] T014 [P] Verify mobile still type-checks against updated `shared/` (`cd mobile && npx tsc --noEmit`) — Constitution Principle I (should be a no-op since `wallet_subtitle` is additive)
- [ ] T015 Run `cd web && npm test` and `cd web && npm run coverage`; ensure ≥90% on changed modules (`MetricCard.tsx`, `ContentHeader.tsx`, `WalletTab.tsx`); run `cd backend && pytest` (should be untouched/green); fix all failures
