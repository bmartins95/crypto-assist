...
2. **Given** a user who previously signed up with Google using the same email address, **When** they authenticate via Facebook, **Then** Cognito's existing attribute-based account linking behavior applies exactly as it already does for Google (no new linking logic is introduced by this feature).
